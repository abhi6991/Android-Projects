...
public class ListAccounts extends Activity  {
	...
	public void onStart() {
		...
		try {
            ...
            Cursor temp = db.rawQuery("Select * from "+Database.ACCOUNTS_TABLE_NAME, null);
      
			if( temp.getCount() == 0){
				this.findViewById(R.id.notfound).setVisibility(View.VISIBLE);
				this.findViewById(R.id.tvTap).setVisibility(View.INVISIBLE);
			}
			else{
				this.findViewById(R.id.notfound).setVisibility(View.INVISIBLE);
				this.findViewById(R.id.tvTap).setVisibility(View.VISIBLE);
